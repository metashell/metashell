...
MetashellInterpreter::MetashellInterpreter(QObject* parent) : QObject(parent)
{
  ...
  connect(
      m_metashell.get(), SIGNAL(finished(int)), this, SLOT(metashellQuit()));
}
  ...
  if (t_json.isObject())
  {
    ...
    if (t_type == "comment")
    {
      ...
      for (QJsonValue it : t_array)
      {
        ...
        QString t_restline =
            t_paragraph["rest_of_lines_indentation"].toString();
    ...
    else if (t_type == "backtrace")
    {
      ...
      for (QJsonValue it : t_object["frames"].toArray())
      {
        ...
        QStringList t_fileinfo =
            t_frameObject["source_location"].toString().split(':');
        ...
        t_fileinfo =
            t_frameObject["point_of_instantiation"].toString().split(':');
        if (t_fileinfo.size() == 3)
        {
    ...
    else if (t_type == "frame")
    {
      ...
      QStringList t_fileinfo =
          t_object["source_location"].toString().split(':');
      if (t_fileinfo.size() == 3)
      {
...
void MetashellInterpreter::metashellQuit() { QApplication::exit(); }
