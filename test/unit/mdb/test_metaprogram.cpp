...
{
  mdb::metaprogram mp(events<metaprogram_mode::normal>(), true);
...
{
  mdb::metaprogram mp(events<metaprogram_mode::full>(), true);
...
{
  mdb::metaprogram mp(events<metaprogram_mode::profile>(), true);
...
TEST(metaprogram, events_are_read_lazily)
{
  ...
  mdb::metaprogram mp(std::move(in_seq_ptr), true);
...
TEST(metaprogram, metaprogram_iterator_reads_events_lazily)
{
  ...
  mdb::metaprogram::iterator i = mp.begin();
  ASSERT_EQ(1, in_seq.next_called_times());

  const mdb::metaprogram::iterator e = mp.end();
  ASSERT_EQ(1, in_seq.next_called_times());
...
TEST(metaprogram, metaprogram_iterator_step_back_from_end_reads_everything)
{
  ...
  mdb::metaprogram::iterator i = mp.end();
  ASSERT_EQ(1, in_seq.next_called_times());
...
{
  mdb::metaprogram mp(build_counting_seq(metaprogram_mode::profile), true);
