...
JUST_TEST_CASE(test_mdb_evaluate_filters_similar_edges) {
  const auto r =
    run_metashell(
      {
        command(fibonacci_with_enum_mp),
        command("#msh mdb"),
        command("evaluate int_<fib<2>::value>"),
    ...
    call_graph(
      {
        ...
        {frame( fib<2>(), _, _, instantiation_kind::template_instantiation), 1, 4},
        {frame(  fib<0>(), _, _, instantiation_kind::memoization), 2, 0},
        {frame(  fib<1>(), _, _, instantiation_kind::memoization), 2, 0},
        {frame(  type("fib<1>::ENUM"), _, _, instantiation_kind::memoization), 2, 0},
