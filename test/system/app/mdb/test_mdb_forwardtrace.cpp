...
{
  const auto r = run_metashell({command("#msh mdb"), command("forwardtrace")});
...
JUST_TEST_CASE(test_mdb_forwardtrace_int)
{
  const auto r = run_metashell(
      {command("#msh mdb"), command("evaluate int"), command("forwardtrace")});

  auto i = r.begin() + 4;

  JUST_ASSERT_EQUAL(call_graph({{frame(type("int")), 0, 1},
                                {frame(type("int"), _, _,
...
JUST_TEST_CASE(test_mdb_forwardtrace_int_in_full_mode)
{
  ...
  JUST_ASSERT_EQUAL(
      call_graph({{frame(type("int")), 0, 1}, {frame(type("int")), 1, 0}}), *i);
}

JUST_TEST_CASE(test_mdb_forwardtrace_when_metaprogram_finished)
{
  const auto r = run_metashell({command("#msh mdb"), command("evaluate int"),
                                command("continue"), command("forwardtrace")});
...
{
  const auto r = run_metashell(
      {command("#msh mdb"), command("evaluate int"), command("ft")});
  ...
  JUST_ASSERT_EQUAL(
      call_graph(
          {{frame(type("int_<fib<5>::value>")), 0, 3},
           ...
           {frame(fib<4>(), _, _, instantiation_kind::template_instantiation),
            2, 2},
...
JUST_TEST_CASE(test_mdb_forwardtrace_from_root_in_full_mode)
{
  ...
  JUST_ASSERT_EQUAL(call_graph({{frame(type("int_<fib<5>::value>")), 0, 2},
                                {frame(fib<5>()), 1, 2},
                                {frame(fib<3>()), 2, 2},
                                {frame(fib<1>()), 3, 0},
                                {frame(fib<2>()), 3, 2},
...
JUST_TEST_CASE(test_mdb_forwardtrace_ft_from_step_1)
{
  const auto r = run_metashell({command(fibonacci_mp), command("#msh mdb"),
                                command("evaluate int_<fib<5>::value>"),
                                command("step"), command("forwardtrace")});
...
JUST_TEST_CASE(test_mdb_forwardtrace_ft_from_step_1_in_full_mode)
{
  ...
  JUST_ASSERT_EQUAL(call_graph({{frame(fib<5>()), 0, 2},
                                {frame(fib<3>()), 1, 2},
                                {frame(fib<1>()), 2, 0},
                                {frame(fib<2>()), 2, 2},
                                {frame(fib<0>()), 3, 0},
...
JUST_TEST_CASE(test_mdb_forwardtrace_ft_from_step_1_with_limit_0)
{
  const auto r = run_metashell({command(fibonacci_mp), command("#msh mdb"),
                                command("evaluate int_<fib<5>::value>"),
                                command("step"), command("forwardtrace 0")});
...
JUST_TEST_CASE(test_mdb_forwardtrace_ft_from_step_1_with_limit_1)
{
  const auto r = run_metashell({command(fibonacci_mp), command("#msh mdb"),
                                command("evaluate int_<fib<5>::value>"),
                                command("step"), command("forwardtrace 1")});
...
JUST_TEST_CASE(test_mdb_forwardtrace_ft_from_step_2_with_limit_2)
{
  const auto r = run_metashell({command(fibonacci_mp), command("#msh mdb"),
                                command("evaluate int_<fib<5>::value>"),
                                command("step"), command("forwardtrace 2")});
...
JUST_TEST_CASE(test_mdb_forwardtrace_ft_from_step_2_with_limit_100)
{
  const auto r = run_metashell({command(fibonacci_mp), command("#msh mdb"),
                                command("evaluate int_<fib<5>::value>"),
                                command("step"), command("forwardtrace 100")});
...
JUST_TEST_CASE(test_mdb_forwardtrace_from_root_on_errored_metaprogram)
{
  const auto r = run_metashell(
      {command(missing_value_fibonacci_mp), command("#msh mdb"),
       command("evaluate int_<fib<5>::value>"), command("forwardtrace")});
...
JUST_TEST_CASE(test_mdb_forwardtrace_when_evaluating_environment_fib_2_3)
{
  const auto r = run_metashell(
      {command(fibonacci_mp + "int_<fib<5>::value> x;"
                              "int_<fib<6>::value> y;"),
       command("#msh mdb"), command("evaluate -"), command("forwardtrace")});
      ...
      call_graph(
          {{frame(type("<environment>")), 0, 8},
           ...
           {frame(type("int_<5>"), _, _,
                  instantiation_kind::template_instantiation),
            1, 0},
           {frame(type("int_<5>"), _, _, instantiation_kind::memoization), 1,
            0},
           ...
           {frame(type("int_<8>"), _, _,
                  instantiation_kind::template_instantiation),
            1, 0},
           {frame(type("int_<8>"), _, _, instantiation_kind::memoization), 1,
            0}}),
      *i);
}
...
{
  const auto r = run_metashell(
      {command(fibonacci_sfinae_mp), command("#msh mdb"),
       command("evaluate decltype(foo<4>())"), command("forwardtrace")});
      ...
      call_graph(
          {{frame(type("decltype(foo<4>())")), 0, 5},
           {frame(type("foo"), _, _,
            ...
            1, 2},
           {frame(
                v2::fib<4>(), _, _, instantiation_kind::template_instantiation),
            2, 2},
           {frame(
                v2::fib<2>(), _, _, instantiation_kind::template_instantiation),
            3, 2},
           {frame(v2::fib<0>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(v2::fib<1>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(
                v2::fib<3>(), _, _, instantiation_kind::template_instantiation),
            3, 2},
           {frame(v2::fib<1>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(v2::fib<2>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(type("enable_if<false, char>"), _, _,
                  instantiation_kind::template_instantiation),
            2, 0},
           {frame(type("foo"), _, _,
                  instantiation_kind::explicit_template_argument_substitution),
            1, 5},
           {frame(
                v1::fib<4>(), _, _, instantiation_kind::template_instantiation),
            2, 2},
           {frame(
                v1::fib<2>(), _, _, instantiation_kind::template_instantiation),
            3, 2},
           {frame(v1::fib<0>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(v1::fib<1>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(
                v1::fib<3>(), _, _, instantiation_kind::template_instantiation),
            3, 2},
           {frame(v1::fib<1>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(v1::fib<2>(), _, _, instantiation_kind::memoization), 4, 0},
           {frame(type("enable_if<true, type-parameter-0-0>"), _, _,
                  instantiation_kind::deduced_template_argument_substitution),
            2, 0},
           {frame(type("enable_if<true, void>"), _, _,
                  instantiation_kind::template_instantiation),
            2, 0},
           ...
           {frame(type("foo"), _, _,
                  instantiation_kind::deduced_template_argument_substitution),
            1, 0},
           {frame(type("foo<4>"), _, _,
                  instantiation_kind::template_instantiation),
            1, 0},
           {frame(type("void"), _, _, instantiation_kind::non_template_type), 1,
            0}}),
      *i);
}
