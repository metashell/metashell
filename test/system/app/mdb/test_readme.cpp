...
JUST_TEST_CASE(test_readme_getting_started)
{
  const auto r = run_metashell(
      {command("template <int N> \\"), command("struct fib \\"),
       command("{ \\"), command("  static constexpr int value = fib<N - "
                                "1>::value + fib<N - 2>::value; \\"),
       command("};"),

       command("template <> \\"), command("struct fib<0> \\"), command("{ \\"),
       command("  static constexpr int value = 1; \\"), command("};"),

       command("template <> \\"), command("struct fib<1> \\"), command("{ \\"),
       command("  static constexpr int value = 1; \\"), command("};"),
       ...
       command("#msh mdb"), command("evaluate int_<fib<6>::value>"),
       command("step 3"), command("step -1"), command("bt"), command("ft"),
       command("rbreak fib<3>"), command("continue"), command("c"), command(""),
       command("e"), command("evaluate -full int_<fib<4>::value>"),
       command("ft")});
  ...
  JUST_ASSERT_EQUAL(
      call_graph(
          {{frame(type("fib<5>"), _, _,
                  instantiation_kind::template_instantiation),
            0, 2},
           {frame(type("fib<4>"), _, _,
                  instantiation_kind::template_instantiation),
            1, 2},
           {frame(type("fib<3>"), _, _,
                  instantiation_kind::template_instantiation),
            2, 2},
           {frame(type("fib<2>"), _, _,
                  instantiation_kind::template_instantiation),
            3, 2},
           ...
           {frame(type("fib<3>"), _, _, instantiation_kind::memoization), 1,
            0}}),
      *i);
  ++i;
  ...
  JUST_ASSERT_EQUAL(prompt("(mdb)"), *i);
  ++i;
  JUST_ASSERT_EQUAL(call_graph({{frame(type("int_<fib<4>::value>")), 0, 2},
...
JUST_TEST_CASE(test_readme_how_to_template_argument_deduction)
{
  const auto r = run_metashell(
      {command("#include <vector>"),
       ...
       command("#msh mdb"), command("evaluate decltype(foo(13))"),
       command("ft"), command("eval decltype(foo(std::vector<int>{}))"),
       command("rbreak foo"), command("continue 2")});
  ...
  JUST_ASSERT(
      frame(type("foo<std::vector<int, std::allocator<int> > >"), _, _,
            instantiation_kind::template_instantiation) == *i ||
      frame(type("foo<std::__1::vector<int, std::__1::allocator<int> > >"), _,
            _, instantiation_kind::template_instantiation) == *i);
  ++i;
...
JUST_TEST_CASE(test_readme_how_to_sfinae)
{
  const auto r = run_metashell(
      {command(make_unique_sfinae_mp),
       command("#msh mdb decltype(make_unique<int>(15))"), command("ft")});
      ...
      call_graph(
          {{frame(type("decltype(make_unique<int>(15))")), 0, 6},
           ...
           {frame(type("make_unique"), _, _,
            ...
            1, 1},
           {frame(
                type("unique_if<int>"), _, _, instantiation_kind::memoization),
            2, 0},
           {frame(type("make_unique"), _, _,
                  instantiation_kind::explicit_template_argument_substitution),
            1, 2},
           ...
           {frame(type("make_unique"), _, _,
                  instantiation_kind::deduced_template_argument_substitution),
            1, 0},
           {frame(type("make_unique<int, int>"), _, _,
                  instantiation_kind::template_instantiation),
            1, 0},
           {frame(type("_std::unique_ptr<int>"), _, _,
                  instantiation_kind::template_instantiation),
            1, 0}}),
      *i);
}
