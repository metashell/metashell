...
namespace
{
  ...
  )
  {
    ...
    JUST_ASSERT_EQUAL(prompt(">"), *i); ++i;
    JUST_ASSERT_EQUAL(prompt(">"), *i); ++i;
    JUST_ASSERT_EQUAL(expected_result_, *i); ++i;
  ...
  json_string generate_warning()
  {
    return command("#warning hello");
  }
}
...
{
  test_definition_and_query(
    command("bool typedef * x;"),
    command("x"),
    type("bool *")
  );
  test_definition_and_query(
    command("char typedef * x;"),
    command("x"),
    type("char *")
  );
  test_definition_and_query(
    command("const typedef int x;"),
    command("x"),
    type("const int")
  );
  test_definition_and_query(
    command("double typedef * x;"),
    command("x"),
    type("double *")
  );
  test_definition_and_query(
    command("float typedef * x;"),
    command("x"),
    type("float *")
  );
  test_definition_and_query(
    command("int typedef * x;"),
    command("x"),
    type("int *")
  );
  test_definition_and_query(
    command("long typedef int x;"),
    command("x"),
    type("long")
  );
  test_definition_and_query(
    command("short typedef int x;"),
    command("x"),
    type("short")
  );
  test_definition_and_query(
    command("signed typedef int x;"),
    command("x"),
    type("int")
  );
  test_definition_and_query(
    command("unsigned typedef int x;"),
    command("x"),
    type("unsigned int")
  );
  test_definition_and_query(
    command("void typedef * x;"),
    command("x"),
    type("void *")
  );
  test_definition_and_query(
    command("volatile typedef int x;"),
    command("x"),
    type("volatile int")
  );
  test_definition_and_query(
    command("wchar_t typedef * x;"),
    command("x"),
    type("wchar_t *")
  );
}
...
JUST_TEST_CASE(test_multiline_input)
{
  ...
  JUST_ASSERT_EQUAL(prompt(">"), *i); ++i;
  JUST_ASSERT_EQUAL(prompt("...>"), *i); ++i;
  JUST_ASSERT_EQUAL(type("const int"), *i); ++i;
