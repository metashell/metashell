...
TEST(mdb_continue, fibonacci_reevaluation_removes_breakpoints)
{
  for (const std::string& nocache1 : nocaches())
  {
    for (const std::string& nocache2 : nocaches())
    {
      metashell_instance mi;
      mi.command(fibonacci_mp);
      mi.command("#msh mdb" + nocache1 + " int_<fib<10>::value>");
      mi.command("rbreak fib<0>");
      mi.command("evaluate" + nocache2 + " int_<fib<10>::value>");

      ASSERT_EQ((std::vector<json_string>{
                    to_json_string(raw_text("Metaprogram finished")),
                    to_json_string(type("int_<55>")),
...
TEST(mdb_continue, fibonacci_2_breakpoints)
{
  for (const std::string& nocache : nocaches())
  {
    int continues_until_fib_5 = 0;

    {
      metashell_instance mi;
      auto continue_ = [&continues_until_fib_5, &mi] {
        ++continues_until_fib_5;
        return mi.command("continue");
      };
      ...
      const std::vector<json_string> r_cont1 = continue_();
      ASSERT_EQ(
          raw_text("Breakpoint 2: regex(\"fib<6>\") reached"), r_cont1[0]);
      ASSERT_EQ(frame(type("fib<6>"), _, _, event_kind::template_instantiation),
                r_cont1[1]);

      std::vector<json_string> r_cont2 = continue_();
      while (r_cont2[0] == raw_text("Breakpoint 2: regex(\"fib<6>\") reached"))
      {
        r_cont2 = continue_();
      }
...

    {
      ...
      const std::vector<json_string> r_cont =
          mi.command("continue " + std::to_string(continues_until_fib_5));
      ASSERT_EQ(raw_text("Breakpoint 1: regex(\"fib<5>\") reached"), r_cont[0]);
      ASSERT_EQ(frame(type("fib<5>"), _, _, event_kind::template_instantiation),
                r_cont[1]);
    }
...
TEST(mdb_continue, minus_1_with_preceding_breakpoint)
{
  ...
  std::vector<json_string> r_cont1 = mi.command("continue");
  while (r_cont1[0] == raw_text("Breakpoint 1: regex(\"fib<6>\") reached"))
  {
    r_cont1 = mi.command("continue");
  }
...
TEST(mdb_continue, to_end_and_back_to_start)
{
  ...
  int step_count = 1;
  std::vector<json_string> r_cont;
  do
  {
    r_cont = mi.command("continue");
    ++step_count;
  } while (raw_text{"Metaprogram finished"} != r_cont[0]);
  ASSERT_EQ(type("int_<55>"), r_cont[1]);

  ASSERT_EQ(raw_text("Metaprogram reached the beginning"),
            mi.command("continue -" + std::to_string(step_count)).front());
}
...
TEST(mdb_continue, to_end_and_back_to_start_in_full_mode)
{
  ...
  ASSERT_EQ(any_of<raw_text>(
                "Breakpoint \"fib<5>\" will stop the execution on 3 locations",
...
{
  const std::string start_mdb = "#msh mdb -full int_<fib<10>::value>";

  metashell_instance mi;
  mi.command(fibonacci_mp);
  mi.command(start_mdb);
  ...
  ASSERT_TRUE(std::regex_match(
      raw_text{mi.command("rbreak fib<5>").front()}.text(), match,
      std::regex{"Breakpoint \"fib<5>\" will stop the execution on ([0-9]+) "
                 "locations"}));

  const std::string steps_to_end = match[1].str();
  ...
  ASSERT_EQ(
      (std::vector<json_string>{
          ...
          to_json_string(prompt("(mdb)"))}),
      mi.command("continue " + steps_to_end));

  ASSERT_EQ(raw_text("Metaprogram reached the beginning"),
            mi.command("continue -" + steps_to_end).front());
}
...
TEST(mdb_step, continue_backwards_fails_without_caching)
{
  ...
  mi.command("#msh mdb -nocache int");
  mi.command("step");
  ASSERT_EQ(caching_backwards_disabled, mi.command("continue -1").front());
}
