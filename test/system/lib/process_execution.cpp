...
int process_execution::exit_code() const
{
  return _exit_code;
}

process_execution metashell_system_test::run(
  std::vector<std::string> cmd_,
  std::string input_
)
{
...
)
{
  return
    out_
      << "Command: " << to_json(e_.cmd()) << std::endl
      << std::endl
      << "Standard input: " << to_json(e_.standard_input()) << std::endl
      << std::endl
      << "Standard output: " << to_json(e_.standard_output()) << std::endl
      << std::endl
