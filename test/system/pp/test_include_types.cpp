...
TEST(include_types, tests)
{
  ...
  const bool std_headers_on_include_path = on_include_path(standard_c_header) ||
  ...
  const include_test_env::result none_found = include_test_env::result::none();
  ...
  for (data::include_type type : {sys, quote})
  {
    ...
    ASSERT_EQ(none_found, env.include_dir_used({}, nonstandard_header, type));

    ASSERT_EQ(system_found, env.include_dir_used({}, standard_c_header, type));

    ASSERT_EQ(
        system_found, env.include_dir_used({}, standard_cpp_header, type));

    if (!std_headers_on_include_path)
    {
      ASSERT_EQ(
          none_found, env.include_dir_used({}, standard_c_header, type,
                                           standard_headers_allowed::none));

      ASSERT_EQ(
          none_found, env.include_dir_used({}, standard_cpp_header, type,
                                           standard_headers_allowed::none));
    }

    if (can_use_c_headers_without_cpp_headers)
    {
      ASSERT_EQ(none_found, env.include_dir_used({}, standard_cpp_header, type,
                                                 standard_headers_allowed::c));

      ASSERT_EQ(
          system_found, env.include_dir_used({}, standard_c_header, type,
                                             standard_headers_allowed::c));
    }
    ...
    ASSERT_EQ(
        capital_i, env.include_dir_used({capital_i}, nonstandard_header, type));

    if (!using_wave())
    {
      ASSERT_EQ(capital_i,
                env.include_dir_used({capital_i}, standard_c_header, type));

      ASSERT_EQ(capital_i,
                env.include_dir_used({capital_i}, standard_cpp_header, type));
    }

    if (!std_headers_on_include_path)
    {
      ASSERT_EQ(
          capital_i, env.include_dir_used({capital_i}, standard_c_header, type,
                                          standard_headers_allowed::none));
    ...
    {
      ASSERT_EQ(
          capital_i, env.include_dir_used({capital_i}, standard_cpp_header,
                                          type, standard_headers_allowed::c));
    ...
    {
      ASSERT_EQ(
          isystem, env.include_dir_used({isystem}, nonstandard_header, type));
    }

    if (!using_msvc() && !using_wave())
    {
      ASSERT_EQ(
          isystem, env.include_dir_used({isystem}, standard_c_header, type));

      ASSERT_EQ(
          isystem, env.include_dir_used({isystem}, standard_cpp_header, type));
    }
    ...
    if (!using_msvc() && !using_wave())
    {
      ...
      ASSERT_EQ(system_found,
                env.include_dir_used({idirafter}, standard_c_header, type));
    ...
    {
      ASSERT_EQ(
          idirafter, env.include_dir_used({idirafter}, standard_cpp_header,
                                          type, standard_headers_allowed::c));
    ...
    if (!using_msvc() && !using_wave())
    {
      ...
      ASSERT_EQ(isystem, env.include_dir_used(
