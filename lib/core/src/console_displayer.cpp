  ...
  namespace core
  {
    namespace
    {
                  ...
                  const std::string& input_filename_)
      {
        ...
        mindent::display(mindent::parse_syntax_node_list(*tokeniser), width_,
                         indent_step_, f_);
      }
      ...
      void show_filenames(InputIt begin_, InputIt end_, pager& pager_)
      {
        ...
        {
          pager_.show(
              data::colored_string(i->string(), data::color::bright_cyan));
          if (!pager_.new_line())
          {
            break;
          }
      ...
      public:
        ...
        typedef std::function<data::colored_string(
    ...
    data::colored_string
    console_displayer::format_token(const data::token& token_)
    {
      return format_code(data::cpp_code(data::format_token(token_)));
    }

    data::colored_string
    console_displayer::format_code(const data::cpp_code& code_)
    {
      if (_syntax_highlight)
      {
    ...
    data::colored_string console_displayer::format_frame(const data::frame& f_)
    {
      ...
      if (f_.is_full())
      {
        postfix << " at " << f_.source_location() << " (" << f_.kind()
                << " from " << f_.point_of_event() << ")";
      }
    ...
    void
    console_displayer::show_file_section(const data::file_location& location_,
                                         const std::string& env_buffer_)
    {
      ...
      int largest_index_length =
          std::to_string(section.back().line_index).size();
