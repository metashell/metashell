...
namespace metashell
{
  namespace data
  {
    ...
    void metaprogram_builder::push_back(event_data event_)
    {
      ...
      case relative_depth::end:
        ...
        if (_frame_stack.empty())
        {
          ...
          {
            for (debugger_event& event : *_events)
            {
        ...
        {
          throw std::runtime_error("Unclosed opening event: " +
    ...
    void metaprogram_builder::push_event(event_data event_)
    {
      _frame_stack.back().add_child();
      _final_bt_pop.flush();

      frame f(std::move(event_), _mode);
      _final_bt->push_front(f);
      _events->emplace_back(std::move(f));
    }

    const type_or_code_or_error& metaprogram_builder::result() const
    {
      return _result;
    }
  }
