...
namespace metashell
{
  namespace data
  {
    ...
    metaprogram_mode metaprogram::get_mode() const { return mode; }
    ...
    void metaprogram::update_backtrace(const debugger_event& event)
    {
      if (current_bt)
      {
        ...
        mpark::visit(
            [this](const auto& e) { this->update_backtrace(e); }, event);
      }
    }

    void metaprogram::update_backtrace(const frame& event)
    {
      current_bt->push_front(event);
    }

    void metaprogram::update_backtrace(const pop_frame& event)
    {
    ...
    const backtrace& metaprogram::get_backtrace()
    {
